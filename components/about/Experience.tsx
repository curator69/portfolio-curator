      ...
      <div className="space-y-4 sm:space-y-6 md:space-y-8">
        {data.map((experience, index) => (
          ...
          >
            {/* Timeline connector */}
            {index < data.length - 1 && (
              <div className="absolute left-12 sm:left-14 md:left-16 top-full w-0.5 h-4 sm:h-6 md:h-8 bg-gradient-to-b from-gray-600 to-transparent"></div>
            )}

            {/* Accordion header - always visible */}
            <div
              className="p-4 sm:p-5 md:p-6 flex flex-col md:flex-row items-center md:items-start gap-3 sm:gap-4 md:gap-6 cursor-pointer relative z-10"
              onClick={() => toggleItem(index)}
            >
              {/* Company logo */}
              <div className="flex-shrink-0">
                <div
                  className={`relative w-16 h-16 sm:w-18 sm:h-18 md:w-20 md:h-20 rounded-full overflow-hidden border-2 flex items-center justify-center transition-all duration-500 ${
                    hoveredItem === index
              ...
              <div className="flex-grow text-center md:text-left flex flex-col gap-1 sm:gap-1.5 md:gap-2">
                <h3
                  className={`text-xl sm:text-xl md:text-2xl font-bold transition-colors duration-300 ${
              ...
              <div className="flex-shrink-0 flex items-center">
                <svg
                  className={`w-5 h-5 sm:w-5 sm:h-5 md:w-6 md:h-6 transition-all duration-500 ${
            ...
            <div
              className={`transition-all duration-500 overflow-hidden ${
                openItems.includes(index)
                  ? "max-h-96 opacity-100 px-4 sm:px-5 md:px-6 pb-4 sm:pb-5 md:pb-6"
                  : "max-h-0 opacity-0"
              }`}
            >
